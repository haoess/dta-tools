...
<xsl:template match='tei:text[not(descendant::tei:text)]'>
  ...
  <xsl:if test='//tei:note[@place="foot"]'>
    ...
    <xsl:apply-templates select='//tei:note[@place="foot" and not(parent::tei:seg[@part="M" or @part="F"])]' mode="footnotes"/>
  </xsl:if>
...
<xsl:template match='tei:g[@ref="#frac"]'>
  ...
  <xsl:element name="img">
    ...
    <xsl:attribute name="src">
      ...
      <xsl:text>http://kaskade.dwds.de/dtaq/formula/preview/</xsl:text><xsl:value-of select="dta:urlencode($fraction)"/>
    </xsl:attribute>
  </xsl:element>
</xsl:template>
...
<xsl:template match='tei:g[@ref="#fric"]'>
  <xsl:variable name="enumerator" select="substring-before(., '/')"/>
  <xsl:variable name="denominator" select="substring-after(., '/')"/>
  <xsl:variable name="fraction" select="concat('\nicefrac{', $enumerator, '}{', $denominator, '}')"/>
  <xsl:element name="img">
    <xsl:attribute name="style">vertical-align:middle; -moz-transform:scale(0.7); -webkit-transform:scale(0.7); transform:scale(0.7)</xsl:attribute>
    <xsl:attribute name="src">
      <xsl:text>http://dinglr.de/formula/</xsl:text><xsl:value-of select="dta:urlencode($fraction)"/>
...
<xsl:template match='tei:note[@place="foot"]' mode="footnotes">
  <div class="footnote">
    <xsl:choose>
      <xsl:when test='parent::tei:seg[@part="M" or @part="F"]'><!--[<xsl:value-of select="@n"/>]--></xsl:when>
...
<xsl:template match='tei:castGroup'>
  <table class="dta-castgroup">
    <xsl:for-each select='tei:castItem'>
      <tr>
        <td class="castitem"><xsl:apply-templates select="tei:role"/></td>
...
<xsl:template match="tei:item">
  <xsl:choose>
    ...
    <xsl:when test="descendant::tei:pb">
      <div class="dta-list-item-noindent"><xsl:apply-templates/></div>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template match='tei:ref'>
  <xsl:element name="span">
    ...
    <xsl:if test="starts-with(@target, '#f') or starts-with(@target, 'BrN3E.htm') or starts-with(@target, 'ZgZuE.htm')">
